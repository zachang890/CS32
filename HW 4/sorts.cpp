//
//  sorts.cpp
//  CS 32 Homework 4 Zachary Chang
//
//  Created by Zachary Chang on 3/1/20.
...
void insertion_sort(vector<Student>& s, bool comp(const Student&, const Student&))
{
    ...
    for (int unsorted = 1; unsorted < s.size(); unsorted++)
    {
        ...
        while ((location > 0) && (!comp(s[location - 1], next)))
        {
...
int main()
{
      ...
      // destruction time.
    {
        ...
        {
            Student* temp = &auxStudents[i];
            studentPointers.push_back(temp);
        }
